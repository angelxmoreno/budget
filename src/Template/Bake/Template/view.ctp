...
<% endif; %>
<?= $this->Html->link(__('Edit <%= $singularHumanName %>'),
    ['action' => 'edit',<%= $pk %>],
    ['class'=>'list-group-item']
) ?>
<?= $this->Form->postLink(__('Delete <%= $singularHumanName %>'),
    ['action' => 'delete',<%= $pk %>],
    ['confirm' => __('Are you sure you want to delete # {0}?', <%= $pk %>),'class'=>'list-group-item']
) ?>
<?= $this->Html->link(__('List <%= $pluralHumanName %>'),
    ['action' => 'index'],
    ['class'=>'list-group-item']
) ?>
...
<div class="panel panel-default">
    <!-- Panel header -->
    <div class="panel-heading">
        <h3 class="panel-title"><?= __('<%= $otherPluralHumanName %>') ?></h3>
    </div>
...
<% $otherPk = "\${$otherSingularVar}->{$details['primaryKey'][0]}"; %>
    ...
    <?php else: ?>
        <p class="panel-body">no <%= $otherPluralHumanName %></p>
    <?php endif; ?>
</div>
<% endforeach; %>
